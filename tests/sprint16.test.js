...
const questions = JSON.parse(await readFile(new URL("../data/questions.json", import.meta.url), "utf8"))
  .filter((question) => !question.questionId.startsWith("Q-N2-FAC-S17-"));
...
const sprint16Version = { ...version, version: "v1.16.0", sprint: "Sprint 16" };
const added = questions.filter((question) => question.questionId.startsWith("Q-N2-FAC-S16-"));
const before = questions.filter((question) => !question.questionId.startsWith("Q-N2-FAC-S16-"));
const selectedIds = new Set(added.flatMap((question) => question.knowledgePointIds));

test("Sprint 16 新增128题并更新版本", () => {
  assert.equal(sprint16Version.version, "v1.16.0");
  assert.equal(sprint16Version.sprint, "Sprint 16");
  assert.equal(added.length, 128);
  assert.equal(questions.length, 577);
...
test("Sprint 16 难度、答案和元数据符合要求", () => {
  const count = (values, value) => values.filter((item) => item === value).length;
  assert.deepEqual([2, 3, 4, 5].map((value) => count(added.map((question) => question.difficulty), value)), [16, 32, 48, 32]);
...
test("Sprint 16 提升考试覆盖率与题库建设进度", () => {
  const sprint15 = buildExamCoverage({ knowledgeCards: cards, questions: before, grammarPoints: grammar });
  const sprint16 = buildExamCoverage({ knowledgeCards: cards, questions, grammarPoints: grammar });
  assert.equal(sprint15.summary.coverageScore, 59.09);
  assert.equal(sprint16.summary.coverageScore, 66.5);
  assert.equal(Math.round(before.length / version.questionTarget * 10000) / 100, 53.14);
  assert.equal(Math.round(questions.length / version.questionTarget * 10000) / 100, 68.28);
});

test("Sprint 16 质量启发式指标不退化", () => {
...
test("Sprint 16 自动报告更新到577题", async () => {
