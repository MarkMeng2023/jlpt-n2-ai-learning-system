...
test("Sprint 14 新增116题并更新版本", () => {
  assert.equal(version.version, "v1.14.0");
  assert.equal(version.sprint, "Sprint 14");
...
test("Sprint 14 报告已更新到321题", async () => {
  ...
  assert.match(exam, /N2 考试覆盖率：\*\*48\.86%\*\*/);
  assert.match(questionCoverage, /当前题目数：\*\*321\*\*/);
  assert.match(questionCoverage, /目标完成率：\*\*37\.99%\*\*/);
  assert.match(plan, /当前目标完成率：\*\*37\.99%\*\*/);
