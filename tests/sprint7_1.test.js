...
test("现有题库覆盖的 10 个 grammar knowledgePoint 均映射到主数据库", () => {
  ...
  const covered = new Set(questions.flatMap((question) => question.knowledgePointIds).filter((id) => pointIds.has(id)));
  assert.equal(covered.size, 10);
  covered.forEach((id) => assert.ok(pointIds.has(id)));
  assert.equal(questions.length, 80, "Sprint 7.1 must not add questions");
});
