...
test("前端加载器可读取新版题库并拒绝缺少旧必需字段的数据", async () => {
  const fetchValid = async (url) => ({
    ok: true,
    status: 200,
    json: async () => url.includes("grammar-points") ? grammarPoints : url.includes("knowledge-points") ? knowledgePoints : questions
  });
  const bank = await loadQuestionBank(fetchValid);
  assert.equal(bank.questions.length, 165);
  ...
  const fetchInvalid = async (url) => ({
    ok: true,
    status: 200,
    json: async () => url.includes("grammar-points") ? grammarPoints : url.includes("knowledge-points") ? knowledgePoints : invalidQuestions
  });
...
test("扩展题库保留旧 questionId，因此历史进度不会重置", () => {
  ...
  assert.deepEqual(counts, { total: 165, completed: 3, remaining: 162 });
});
