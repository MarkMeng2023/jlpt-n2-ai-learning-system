...
const generated = questions.filter((question) => question.generationType === "question_factory");
...
test("Sprint 9 仅为 Factory Phase 1 Top 10 新增45题", () => {
  ...
  assert.deepEqual(validateQuestionBank(questions, points), { valid: true, errors: [] });
});
...
test("页面可见版本号更新为 Sprint 9", async () => {
  const html = await readFile(new URL("../index.html", import.meta.url), "utf8");
  assert.match(html, /JLPT N2 · Sprint 9/);
});
