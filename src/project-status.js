function requireVersion(version) {
  ...
  if (!Number.isInteger(version.questionTarget) || version.questionTarget <= 0) throw new Error("Question Target 必须是正整数");
  if (typeof version.lastUpdated !== "string" || Number.isNaN(Date.parse(version.lastUpdated))) throw new Error("Last Updated 必须是 ISO 时间");
}

export function buildProjectStatus({ questions, basePoints, grammarPoints, version }) {
...
export async function loadProjectStatusData(questions, basePoints, fetchImpl = globalThis.fetch, loadedGrammarPoints = null) {
  ...
  return buildProjectStatus({ questions, basePoints, grammarPoints, version });
}
